...
def lists2object(foodnames:List[str], quantities:List[str], unit_list:List[str], taxons:List[str]) -> List[Dict]:
...
def str2ngrams(string:str) -> List[str]:
    ...
    if len(token_list)>2:
        ngrams += [w1+' '+w2+' '+w3 for w1,w2,w3 in zip(token_list[:-2], token_list[1:-1], token_list[2:])] # trigrams
    if len(token_list)>1:
        ngrams += [w1+' '+w2 for w1,w2 in zip(token_list[:-1], token_list[1:])] # bigrams
    return ngrams
...
class Shelf(object):
    ...
    def __init__(self, region:str='EU', lang_source:str='un', month_id:int=0):
        ...
        self.region, self.lang_source, self.month_id = region, lang_source.lower(), month_id

        with open(os.path.join(prefix,'assets','vocab','mapping.json'), 'r') as fp:
            mapping = json.load(fp) # food univeral mapping (f2id, id2f)
            self.mapping = mapping[self.lang_source] # load session's language for basket of food (BOW) representation

        with open(os.path.join(prefix,'assets','vocab','feats.json'), 'r') as fp:
            self.feats = json.load(fp) # food attributes {food_id: {'taxon': , 'fr': , 'es': , 'en': , 'default_weight': , 'default_density': , 'benefits/diet': , nutri...}}
            self.n_ingredients = len(self.feats) # total number of ingredients
    
        self.seasonal_matrix = load_npz(os.path.join(prefix,'assets','seasons','sparse_seasons_{}.npz'.format(region))) # domain expert knowledge (n_seasons, n_food) varies by 'regions'
        self.seasonal_vector = np.ravel(self.seasonal_matrix[self.month_id].toarray()) # local and seasonal ingredients   ##### map2producers
    ...
    def get_seasonal_food(self, key:str='001') -> List[str]:
        ...
        seasonal_food = []
        for fid in np.argwhere(self.seasonal_vector>0.):
    ...
    def get_urls(self, food_list:List[str]) -> List[str]:
        return [self.feats[str(self.mapping['f2id'][foodname.lower()])]['url'] for foodname in food_list]
    ...
    def load_tfidf(self):
        ...
        if self.tfidf is None:
            self.tfidf = pickle.load(open(os.path.join(prefix,'assets','vocab','{}/tfidf_{}.pickle'.format(self.lang_source, self.lang_source)), "rb")) # ngram vectorizer for food names
            self.ngrams = pickle.load(open(os.path.join(prefix,'assets','vocab','{}/ngrams_{}.pickle'.format(self.lang_source, self.lang_source)), "rb"))
    ...
    def get_food_info(self, food_name:str) -> Tuple:
        ...
        fid = self.mapping['f2id'][food_name] # ref id, for example: Reine Claude --> Prune
        taxon = self.feats[str(fid)]['taxon']
        score = self.seasonal_vector[int(fid)]
        return food_name, fid, taxon, score

    def text2food(self, food_name:str='apple', threshold:float=0.) -> Tuple:
        """
        ...
        food_name = urllib.parse.unquote(food_name) # replace %xx escapes by their single-character equivalent (utf-8 encoding)
        food_name = re.sub(r"\(.*\)", "", food_name).lower().strip() # remove parenthesis, lower case and strip string
        if food_name in self.mapping['f2id']:
            food_name, fid, taxon, score = self.get_food_info(food_name=food_name)
            return food_name, fid, taxon, score

        food_name_az = unidecode.unidecode(food_name) # convert unicode with accents to unaccented_string
        food_name_az = re.sub('[^a-zA-Z]+', ' ', food_name_az).strip() # replace apostrophees, non alpha characters by spaces and strip string
        ...
        affinity = penalty*cosine_similarity(self.tfidf.transform(candidates), self.ngrams, dense_output=True) # (len(candidates), n_ingredients)
    ...
    def convert2g(self, fid:float, qty:float, unit:str) -> float:
        ...
        if unit in units['misc']:
            return qty*float(units['misc'][unit])*self.feats[str(fid)]['default_weight'] 
        return qty*self.feats[str(fid)]['default_weight']

    def NER(self, food_list:List[str], qty_list:Optional[List[str]], unit_list:Optional[List[str]], lang_dest:Optional[str], filter_HS:bool=True) -> Tuple:
        ...
        recipe_vector = np.zeros((self.n_ingredients,1)) # basket of food (n_ingredients,1)
        foodnames, qties, units, taxons = [], [], [], [] # extracted entity (length < food_list)
        ...
        for (food_,qty_,unit_) in zip(food_list,qty_list,unit_list): # ingredients
            ...
            if fid is not None: # food_ is in food vocab
                ...
                if filter_HS and (score<0.5 or taxon.startswith('21')): # seasonality threshold below 0.5 or non vege ingredient (i.e. fish, meat, egg, seafood)
                    HS.append(food_)
                    HS_id.append(fid)
                else:
                    qties.append(qty_)
    ...
    def process_ingredients(self, food_list:List[str], qty_list:Optional[List[str]], unit_list:Optional[List[str]], lang_dest:Optional[str], revisit:bool=False, infer_nutri:bool=False, serving:int=1):
        ...
        # read recipe
        my_recipe_vector, foodnames, qties, unit_list, taxons, HS, HS_id = self.NER(food_list=food_list, qty_list=qty_list, unit_list=unit_list, lang_dest=lang_dest) # map ingredients to basket of food
        self.tags = {} # init tags
        self.tags['ingredients'] = lists2object(foodnames, qties, unit_list, taxons) # seasonal and vege ingedients
        self.tags['HS'] = HS # rest of ingredients

        self.tags['ingredients_by_taxon'] = {} # dict of list, e.g. 001: [pomme, poire], 101: [sel]
        ...
        self.tags['ingredients_by_taxon'] = [self.tags['ingredients_by_taxon'][taxon] for taxon in np.sort(np.unique(taxons))] # list of list, e.g. [ [pomme, poire], [sel] ]

        # infer seasonality
        if my_recipe_vector.sum()==0:
            self.tags['labels'] = {'vegan': False, 'vege':False, 'seasonality': {'region':self.region, 'score':0, 'best_now':False}}
            self.tags['revisited'] = []
            if infer_nutri==True:
                self.tags['nutri'] = {'Allergies':{}, 'Energy':{}, 'Macro':{}, 'Minerals':{}, 'Vitamines':{}}

        else:
            my_recipe_vector /= my_recipe_vector.sum() # normalize ingredients (proportions) for seasonality
            my_seasons_vector = (self.seasonal_matrix * csr_matrix(my_recipe_vector)).toarray() # infer seasonality from seasonal ingredients: (n_seasons,n_ingredients) * (n_ingredients,1) = (n_seasons,1)
            seasonality = {'city': self.region, 'score':int(100*my_seasons_vector[self.month_id][0]), 'best_now':len(HS)==0}

            # infer labels
            is_vege = len([1 for fid in HS_id if self.feats[str(fid)]['taxon'].startswith('21')])==0 # Vegetarian label
            is_vegan = is_vege and len([1 for taxon in taxons if taxon.startswith('2')])==0 # Vegan label (check no diary and cheese)
            self.tags['labels'] = {'vegan':is_vegan, 'vege':is_vege, 'seasonality':seasonality}
            ...
            if infer_nutri==True:
                ...
                nutriscores = load_npz(os.path.join(prefix,'assets','nutri','sparse_nutriscores.npz')) # load nutriscores matrix
                ...
                for c in ['Energy', 'Macro', 'Minerals', 'Vitamins']:
                    for nutri_name, nutri_id in nutrimap[c].items():
                        composcore = 100*float(my_nutri_vector[nutri_id])/vnr[c][nutri_name] # infer kCal (energy), macro scores, minerals scores and vitamines scores (% VNR) + Allergies (0 ou 1)
                        if composcore>0:
