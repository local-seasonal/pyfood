...
def test_vocab(assets_paths="pyfood/assets"):
    ...
    assert (
        "es" in mapping.keys()
        and "fr" in mapping.keys()
        and "en" in mapping.keys()
        and "it" in mapping.keys()
    )
    assert len(mapping["fr"]["id2f"]) >= len(
        mapping["fr"]["f2id"]
    )  # synonyms id2f > f2id
    ...
    assert (
        "default_weight" in feats["0.0"]
        and "density" in feats["0.0"]
        and "allergen" in feats["0.0"]
        and "url" in feats["0.0"]
    )
    assert (
        "default_weight" in feats["0.0"]
        and "density" in feats["0.0"]
    ...
    assert (
        "CIQUAL_ref" in feats["0.0"]
        and "kCal" in feats["0.0"]
        and "macro" in feats["0.0"]
        and "minerals" in feats["0.0"]
        and "vitamins" in feats["0.0"]
    )  # from CIQUAL
...
def test_nutriscores(assets_paths="pyfood/assets"):
    ...
    assert (
        "Energy" in nutrimap
        and "Macro" in nutrimap
        and "Minerals" in nutrimap
        and "Vitamins" in nutrimap
    )

    with open(os.path.join(assets_paths, "vocab/feats.json"), "r") as fp:
        feats = json.load(fp)

    dense_scores = load_npz(
        os.path.join(assets_paths, "nutri/sparse_nutriscores.npz")
    ).toarray()  # nutri info from CIQUAL
...
def test_seasonality(assets_paths="pyfood/assets"):
    ...
    for region in ["Japan", "France", "Spain", "Senegal", "Canada", "United Kingdom"]:
        sparse_seasonality = load_npz(
            os.path.join(assets_paths, "seasons/sparse_seasons_{}.npz".format(region))
        )  #  csr_matrix((data, (row, col)), shape=(12, n_ingredients), dtype=np.float) # ingredients <-> seasons
...
def test_text2food():
    ...
    foodname, fid, taxon, score = shelf.text2food(
        food_name="Tomates cerise", threshold=0.0
    )
    assert foodname == "tomate cerise"
    assert taxon == "001"  # fruit

    # test text2food
    foodname, fid, taxon, score = shelf.text2food(food_name="carotttte", threshold=0.0)
    assert foodname == "carotte"
    assert taxon == "002"

    foodname, fid, taxon, score = shelf.text2food(
        food_name="250± g de shrim&ps βßdécortiquées (surgelées)", threshold=0.0
    )
    assert foodname == "shrimp"
    assert taxon.startswith("2")  # non vege

    foodname, fid, taxon, score = shelf.text2food(food_name="Lasagne", threshold=0.0)
    assert foodname == "lasagne"

    foodname, fid, taxon, score = shelf.text2food(food_name="Bechamel", threshold=0.0)
    assert foodname == "bechamel"

    foodname, fid, taxon, score = shelf.text2food(food_name="Zuchini", threshold=0.0)
    assert foodname == "zucchini"

    foodname, fid, taxon, score = shelf.text2food(food_name="Apple±eee", threshold=0.0)
    assert foodname == "apple"

    foodname, fid, taxon, score = shelf.text2food(
        food_name="300g pommes (vertes)", threshold=0.0
    )
...
def test_process_ingredients():
    ...
    my_recipe_vector, foodnames, qties, unities, taxons, HS, HS_id = shelf.NER(
        food_list=["tomates", "kiwi", "sucre"],
        qty_list=["1", "1", "1"],
        unit_list=["kg", "default", "g"],
        lang_dest=None,
    )
    assert "tomate" in foodnames
    assert "kiwi" not in foodnames

    recipe_vector, foodnames, qties, units, taxons, HS, HS_id = shelf.NER(
        food_list=["apple"], qty_list=None, unit_list=None, lang_dest=None
    )
    assert foodnames == ["apple"]

    # test process_ingredients
    shelf.process_ingredients(
        food_list=["poire", "pomme", "cerise", "fraise"],
        qty_list=["1", "1", "1"],
        unit_list=["g", "g", "g"],
        lang_dest="fr",
        infer_nutri=True,
    ...
    assert (
        shelf.tags["labels"]["seasonality"]["score"] >= 0.0
        and shelf.tags["labels"]["seasonality"]["score"] <= 100.0
    )
